...
import { parseProfileId, type ProfileId } from './types.ts';
...
export function readProfileOverride(data: Record<string, unknown>): ProfileId | null {
  const meta: unknown = data['palantir_mcp'];
  if (!isRecord(meta)) return null;
  return parseProfileId(meta['profile']);
}

export function setProfileIfMissing(data: Record<string, unknown>, profile: ProfileId): void {
  const meta: Record<string, unknown> = ensureObject(data, 'palantir_mcp');
  const existing: unknown = meta['profile'];
